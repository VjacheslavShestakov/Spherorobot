...
void Freq3(int );
void Freq4(int );
void Motor_All(int, int, int, int, int, int, int, int);
...
//K4_EN=GPIO_Pin_10,
//K4_RESET=GPIO_Pin_11,
K4_SLEEP=GPIO_Pin_10,
K4_DIR=GPIO_Pin_11,
//K4FLT=GPIO_Pin_14,
...
/////////////////////////////////////////////////////////////////////////////////
// for UART's bufer 
#define NOM 15	//number of receiving bytes by UART
...
void InitTimer7(int period)
{
  ...
  TIM_Cmd(TIM7, ENABLE);	  //вкл таймер
}
...
/*
  ...
  GPIO_Init(GPIOA, &gpio);
	...
	gpio.GPIO_Mode = GPIO_Mode_OUT;
	gpio.GPIO_OType = GPIO_OType_PP;
	gpio.GPIO_Pin = GPIO_Pin_1 | GPIO_Pin_2 | GPIO_Pin_3| GPIO_Pin_0;
	gpio.GPIO_Speed = GPIO_Speed_50MHz;
  GPIO_Init(GPIOA, &gpio);
}
...
void obrabotka_bufera(void)
{
	...
	{
			if((buffer[0]==2)&&(buffer[1]==125))	// if we received speeds of motors
			{
				// wheel 1
								
				speed1=buffer[3]<<8;				// rotational velocity of the 1 wheel, RPM
				speed1+=buffer[4];
				
				
				// wheel 2
				speed2=buffer[6]<<8;				// rotational velocity of the 2 wheel, RPM
				speed2+=buffer[7];																			
				
				
				// wheel 3
				speed3=buffer[9]<<8;				// rotational velocity of the 3 wheel, RPM
				speed3+=buffer[10];	

				// wheel 4
				speed4=buffer[12]<<8;				// rotational velocity of the 3 wheel, RPM
				speed4+=buffer[13];
				
								
				// direction of moving
				if(buffer[14]==1)	//forward
				{
					USART_Transmit(1);
					ss1=speed1;
					ss2=speed2;
					ss3=speed3;
					ss4=speed4;
					Motor_All(buffer[2], speed1, buffer[5], speed2, buffer[8], speed3, buffer[11], speed4);
				}
				else if(buffer[14]==0)	//stop
				{
					Motor_All(0, ss1, 0, ss2, 0, ss3, 0, ss4);
					USART_Transmit(0);
					
				}
				if(buffer[14]==2)	//rhombus
				{
					
					//rhombus
					Motor_All(1, 0, 1, 0, 2, 0, 2, speed1);
					delay(1000);
					Motor_All(0, 0, 0, speed1, 0, 0, 0, speed1);
					delay(500);
					Motor_All(2, speed1, 2, 0, 1, speed1, 1, 0);
					delay(1000);
					Motor_All(0, speed1, 0, 0, 0, speed1, 0, 0);
					delay(500);
						Motor_All(2, 0, 2, speed1, 1, 0, 1, speed1);
					delay(1000);
					Motor_All(0, 0, 0, speed1, 0, 0, 0, speed1);
					delay(500);
					Motor_All(1, speed1, 1, 0, 2, speed1, 2, 0);
					delay(1000);
					Motor_All(0, speed1, 0, 0, 0, speed1, 0, 0);
				...
				/*square
				Motor_All(1, sp, 2, sp, 1, sp, 2, sp);
				delay(1000);
				Motor_All(0, sp, 0, sp, 0, sp, 0, sp);
				delay(500);
				Motor_All(2, sp, 2, sp, 1, sp, 1, sp);
				delay(1000);
				Motor_All(0, sp, 0, sp, 0, sp, 0, sp);
				delay(500);
				Motor_All(2, sp, 1, sp, 2, sp, 1, sp);
				delay(1000);
				Motor_All(0, sp, 0, sp, 0, sp, 0, sp);
				delay(500);
				Motor_All(1, sp, 1, sp, 2, sp, 2, sp);
				delay(1000);
				Motor_All(0, sp, 0, sp, 0, sp, 0, sp);
				*/
...
void Motor_All(int dir1, int freq1, int dir2, int freq2, int dir3, int freq3, int dir4, int freq4)
{
	// direction = 0 -> stop
	// direction = 1 -> forward
	// direction = 2 -> back
	...
	int number_points;
	int start_speed1, start_speed2, start_speed3, start_speed4;
	int break_speed;
	int temp;
	...
	if(dir1 == 1)
	{
   	GPIO_SetBits(GPIOB, K1_DIR);
		GPIO_ResetBits(GPIOB, K1_SLEEP);
	}
	else if(dir1 == 2)
	{
		
		GPIO_ResetBits(GPIOB, K1_SLEEP|K1_DIR);
	}
	else if(dir1 == 0)
	{
		//Freq1(0);
	...
	if(dir2 == 1)
	{
   	GPIO_SetBits(GPIOB, K2_DIR);
		GPIO_ResetBits(GPIOB, K2_SLEEP);
	}
	else if(dir2 == 2)
	{
		
		GPIO_ResetBits(GPIOB, K2_SLEEP|K2_DIR);
	}
	else if(dir2 == 0)
	{
		//Freq2(0);
	...
	if(dir3 == 1)
	{
   	GPIO_SetBits(GPIOB, K3_DIR);
		GPIO_ResetBits(GPIOB, K3_SLEEP);
	}
	else if(dir3 == 2)
	{
		
		GPIO_ResetBits(GPIOB, K3_SLEEP|K3_DIR);
	}
	else if(dir3 == 0)
	{
		//Freq3(0);
	...
	if(dir4 == 1)
	{
   	GPIO_SetBits(GPIOB, K4_DIR);
		GPIO_ResetBits(GPIOB, K4_SLEEP);
	}
	else if(dir4 == 2)
	{
		
		GPIO_ResetBits(GPIOB, K4_SLEEP|K4_DIR);
	}
	else if(dir4 == 0)
	{
		//Freq3(0);
...
///////////////////////////////////////////////////////////////////////////////////////////////////
	...
	start_speed4=1000000/(2*((short)(TIM7->ARR)+1));
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//Calculate k1, k2, k3, k4 for acceleration and braking
	if(dir1 != 0)
	{
	...
	if(dir4 !=0)
	{
		k4=(freq4-start_speed4)/number_points;
	}
	else
	{
		temp = 1000000/(2*(TIM7->ARR+1));
		if(temp<200)
		{
			k4=0;
		}
		...
		{
			k4= ( temp-break_speed )/number_points;
		}
	}
///////////////////////////////////////////////////////////////////////////////////////////////////
...
///////////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(freq4<break_speed && start_speed4==0)
	{
		spd4=freq4;
		k4=0;
	}
